...
type AtributoLugarController struct {
	beego.Controller
}

// URLMapping ...
func (c *AtributoLugarController) URLMapping() {
...
// @Param	body		body 	models.AtributoLugar	true		"body for AtributoLugar content"
// @Success 201 {int} models.AtributoLugar
...
// @Success 200 {object} models.AtributoLugar
...
// @Failure 403
// @router / [get]
func (c *AtributoLugarController) GetAll() {
	var fields []string
	var sortby []string
	var order []string
	...
	l, err := models.GetAllAtributoLugar(query, fields, sortby, order, offset, limit)
	if err != nil {
		c.Data["json"] = models.Alert{Type: "error", Code: "E_400", Body: err.Error()}
	} else {
		c.Data["json"] = l
...
// @router /:id [put]
func (c *AtributoLugarController) Put() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v := models.AtributoLugar{Id: id}
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if err := models.UpdateAtributoLugarById(&v); err == nil {
...
// @Failure 403 id is empty
// @router /:id [delete]
func (c *AtributoLugarController) Delete() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	if err := models.DeleteAtributoLugar(id); err == nil {
